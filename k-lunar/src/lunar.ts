// @ts-nocheck
import { KASI_HISTORICAL_MONTHS } from "./kasiHistoricalData";

const factory = function(){
  // KASI month-start bridge for the supported conversion range.
  var _KASI_HISTORICAL_LUNAR_INDEX = null;
  var _KASI_HISTORICAL_SOLAR_START = {
    year: -59,
    month: 2,
    day: 13
  };
  var _KASI_HISTORICAL_SOLAR_END = {
    year: 2050,
    month: 12,
    day: 31
  };
  var _KASI_HISTORICAL_LUNAR_YEAR_START = -59;
  var _KASI_HISTORICAL_LUNAR_YEAR_END = 2050;
  var _SOLAR_DAYS_OF_MONTH = [31,28,31,30,31,30,31,31,30,31,30,31];
  var _KST_OFFSET_MINUTES = 9 * 60;
  var _KST_OFFSET_MILLIS = _KST_OFFSET_MINUTES * 60 * 1000;

  // Shared numeric validation helpers used by solar/lunar input paths.
  var _toNumberOrThrow = function(value, errorPrefix) {
    var original = value;
    value *= 1;
    if (isNaN(value)) {
      throw new Error(errorPrefix + original);
    }
    return value;
  };
  var _assertRange = function(value, min, max, errorPrefix) {
    if (value < min || value > max) {
      throw new Error(errorPrefix + value);
    }
  };
  var _normalizeClock = function(hour, minute, second) {
    hour = _toNumberOrThrow(hour, 'wrong hour ');
    minute = _toNumberOrThrow(minute, 'wrong minute ');
    second = _toNumberOrThrow(second, 'wrong second ');
    _assertRange(hour, 0, 23, 'wrong hour ');
    _assertRange(minute, 0, 59, 'wrong minute ');
    _assertRange(second, 0, 59, 'wrong second ');
    return {
      hour: hour,
      minute: minute,
      second: second
  ...
  var _normalizeSolarInput = function(year, month, day, hour, minute, second) {
    var time = _normalizeClock(hour, minute, second);
    year = _toNumberOrThrow(year, 'wrong solar year ');
    month = _toNumberOrThrow(month, 'wrong solar month ');
    day = _toNumberOrThrow(day, 'wrong solar day ');
    _assertRange(month, 1, 12, 'wrong month ');
    _assertRange(day, 1, 31, 'wrong day ');
    return {
      year: year,
      month: month,
      day: day,
  ...
  var _normalizeLunarInput = function(year, month, day, hour, minute, second) {
    var time = _normalizeClock(hour, minute, second);
    year = _toNumberOrThrow(year, 'wrong lunar year ');
    month = _toNumberOrThrow(month, 'wrong lunar month ');
    day = _toNumberOrThrow(day, 'wrong lunar day ');
    return {
      year: year,
      month: month,
      day: day,
  ...
  var _normalizeSolarDate = function(year, month, day) {
    year = _toNumberOrThrow(year, 'wrong solar year ');
    month = _toNumberOrThrow(month, 'wrong solar month ');
    day = _toNumberOrThrow(day, 'wrong solar day ');
    return {
  ...
  // Proleptic Gregorian policy helpers used by SolarUtil and date movement.
  var _isGregorianLeapYear = function(year) {
    year = _toNumberOrThrow(year, 'wrong solar year ');
    return (year % 4 === 0 && year % 100 !== 0) || (year % 400 === 0);
  };
  var _getSolarDaysOfMonth = function(year, month) {
    year = _toNumberOrThrow(year, 'wrong solar year ');
    month = _toNumberOrThrow(month, 'wrong solar month ');
    var m = month - 1;
    var days = _SOLAR_DAYS_OF_MONTH[m];
    if (m === 1 && _isGregorianLeapYear(year)) {
      days++;
    }
    return days;
  };
  var _getSolarDaysOfYear = function(year) {
    return _isGregorianLeapYear(year) ? 366 : 365;
  };
  var _getSolarDaysInYear = function(year, month, day) {
    var normalized = _normalizeSolarDate(year, month, day);
    year = normalized.year;
    month = normalized.month;
    day = normalized.day;
    var days = 0;
    for (var i = 1; i < month; i++) {
      days += _getSolarDaysOfMonth(year, i);
    }
    days += day;
    return days;
  };
  var _getSolarDaysBetween = function(ay, am, ad, by, bm, bd) {
    var from = _normalizeSolarDate(ay, am, ad);
    var to = _normalizeSolarDate(by, bm, bd);
    ay = from.year;
    am = from.month;
    ad = from.day;
    by = to.year;
    ...
    var i;
    if (ay === by) {
      n = _getSolarDaysInYear(by, bm, bd) - _getSolarDaysInYear(ay, am, ad);
    } else if (ay > by) {
      days = _getSolarDaysOfYear(by) - _getSolarDaysInYear(by, bm, bd);
      for (i = by + 1; i < ay; i++) {
        days += _getSolarDaysOfYear(i);
      }
      days += _getSolarDaysInYear(ay, am, ad);
      n = -days;
    } else {
      days = _getSolarDaysOfYear(ay) - _getSolarDaysInYear(ay, am, ad);
      for (i = ay + 1; i < by; i++) {
        days += _getSolarDaysOfYear(i);
      }
      days += _getSolarDaysInYear(by, bm, bd);
      n = days;
    }
    return n;
  };
  var _shiftSolarDate = function(year, month, day, days) {
    year = _toNumberOrThrow(year, 'wrong solar year ');
    month = _toNumberOrThrow(month, 'wrong solar month ');
    day = _toNumberOrThrow(day, 'wrong solar day ');
    days = _toNumberOrThrow(days, 'wrong offset ');
    while (days > 0) {
      day += 1;
      if (day > _getSolarDaysOfMonth(year, month)) {
        day = 1;
        month += 1;
        if (month > 12) {
          month = 1;
    ...
    while (days < 0) {
      ...
      if (day < 1) {
        ...
        day = _getSolarDaysOfMonth(year, month);
      }
      days += 1;
    }
    return {
  ...
  var _isKasiHistoricalSolarSupported = function(year, month, day) {
    var target = {
      year: year,
      month: month,
      day: day
    };
    return _compareDate(target, _KASI_HISTORICAL_SOLAR_START) >= 0 && _compareDate(target, _KASI_HISTORICAL_SOLAR_END) <= 0;
  };
  var _isKasiHistoricalLunarSupported = function(year) {
    return year >= _KASI_HISTORICAL_LUNAR_YEAR_START && year <= _KASI_HISTORICAL_LUNAR_YEAR_END;
  };
  var _getKasiHistoricalLunarIndex = function() {
    if (_KASI_HISTORICAL_LUNAR_INDEX) {
      return _KASI_HISTORICAL_LUNAR_INDEX;
    }
    var index = {};
    for (var i = 0, j = KASI_HISTORICAL_MONTHS.length; i < j; i++) {
      var month = KASI_HISTORICAL_MONTHS[i];
      index[month[3] + '/' + month[4]] = month;
    }
    _KASI_HISTORICAL_LUNAR_INDEX = index;
    return index;
  };
  var _getKasiHistoricalLunarBySolar = function(year, month, day) {
    if (!_isKasiHistoricalSolarSupported(year, month, day)) {
      return null;
    }
    var target = {
      year: year,
    ...
    var left = 0;
    var right = KASI_HISTORICAL_MONTHS.length - 1;
    var matchedIndex = -1;
    while (left <= right) {
      var middle = Math.floor((left + right) / 2);
      var current = KASI_HISTORICAL_MONTHS[middle];
      var comparison = _compareDate(
        {
          year: current[0],
          month: current[1],
          day: current[2]
    ...
    var matched = KASI_HISTORICAL_MONTHS[matchedIndex];
    var offset = _getSolarDaysBetween(matched[0], matched[1], matched[2], year, month, day);
    if (offset < 0 || offset >= matched[5]) {
      return null;
    }
    return {
      year: matched[3],
      month: Math.abs(matched[4]),
      day: offset + 1,
      intercalation: matched[4] < 0
    };
  };
  var _getKasiHistoricalSolarByLunar = function(year, month, day, isLeapMonth) {
    if (!_isKasiHistoricalLunarSupported(year)) {
      return null;
    }
    var signedMonth = isLeapMonth ? -month : month;
    var monthEntry = _getKasiHistoricalLunarIndex()[year + '/' + signedMonth];
    if (!monthEntry) {
      return null;
    }
    if (day < 1 || day > monthEntry[5]) {
      return null;
    }
    return _shiftSolarDate(monthEntry[0], monthEntry[1], monthEntry[2], day - 1);
  };
  var Solar = (function(){
    // Solar-internal movement and Julian day conversion helpers.
    var _normalizeDelta = function(value, errorPrefix) {
      var original = value;
      value *= 1;
      if (isNaN(value)) {
        throw new Error(errorPrefix + original);
    ...
    var _rebalanceClockFromJulianDay = function(day, hour, minute, second) {
      if(second>59){
        second-=60;
        minute++;
      }
    ...
    var _getSolarPartsFromJulianDay = function(julianDay) {
      var d = Math.floor(julianDay + 0.5);
      var f = julianDay + 0.5 - d;
      var c = Math.floor((d - 1867216.25) / 36524.25);
      ...
      var rebalanced = _rebalanceClockFromJulianDay(day, hour, minute, second);
      return {
        year: year,
        month: month,
        day: rebalanced.day,
        hour: rebalanced.hour,
        minute: rebalanced.minute,
        second: rebalanced.second
      };
    };
    var _getJulianDayFromSolarParts = function(year, month, day, hour, minute, second) {
      var normalized = _normalizeSolarInput(year, month, day, hour, minute, second);
      year = normalized.year;
      month = normalized.month;
      day = normalized.day;
      ...
      var julianDay = day + ((second / 60 + minute) / 60 + hour) / 24;
      if (month <= 2) {
        month += 12;
        year--;
      }
      var century = Math.floor(year / 100);
      century = 2 - century + Math.floor(century / 4);
      return Math.floor(365.25 * (year + 4716)) + Math.floor(30.6001 * (month + 1)) + julianDay + century - 1524.5;
    };
    var _shiftSolarYears = function(solar, years) {
      var y = solar.getYear() + years;
      var m = solar.getMonth();
      var d = solar.getDay();
      if (2 === m && d > 28 && !_isGregorianLeapYear(y)) {
        d = 28;
      }
      return _fromYmdHms(y, m, d, solar.getHour(), solar.getMinute(), solar.getSecond());
    };
    var _shiftSolarMonths = function(solar, months) {
      var month = SolarMonth.fromYm(solar.getYear(), solar.getMonth()).next(months);
      var y = month.getYear();
      var m = month.getMonth();
      var d = solar.getDay();
      var maxDay = _getSolarDaysOfMonth(y, m);
      if (d > maxDay) {
        d = maxDay;
      }
      return _fromYmdHms(y, m, d, solar.getHour(), solar.getMinute(), solar.getSecond());
    };
    var _shiftSolarDays = function(solar, days) {
      var y = solar.getYear();
      var m = solar.getMonth();
      var d = solar.getDay();
      if (days > 0) {
        d += days;
        var daysInMonth = _getSolarDaysOfMonth(y, m);
        while (d > daysInMonth) {
          d -= daysInMonth;
          m++;
          if (m > 12) {
          ...
          daysInMonth = _getSolarDaysOfMonth(y, m);
        }
      } else if (days < 0) {
        while (d + days <= 0) {
          m--;
          ...
          d += _getSolarDaysOfMonth(y, m);
        }
        d += days;
      }
      return _fromYmdHms(y, m, d, solar.getHour(), solar.getMinute(), solar.getSecond());
    };
    var _fromDatePartsInFixedOffset = function(date, offsetMillis){
      var time = date.getTime();
      if (isNaN(time)) {
        throw new Error('wrong date ' + date);
      }
      var shifted = new Date(time + offsetMillis);
      return _fromYmdHms(
    ...
    var _fromDate = function(date){
      return _fromDatePartsInFixedOffset(date, _KST_OFFSET_MILLIS);
    };
    var _fromJulianDay = function(julianDay){
      var solarParts = _getSolarPartsFromJulianDay(julianDay);
      return _fromYmdHms(solarParts.year, solarParts.month, solarParts.day, solarParts.hour, solarParts.minute, solarParts.second);
    };
    var _fromYmdHms = function(y,m,d,hour,minute,second){
      var normalized = _normalizeSolarInput(y, m, d, hour, minute, second);
      return {
        _p:{
          year:normalized.year,
          month:normalized.month,
        ...
        /**
         * 해당 날짜가 속한 양력 주를 가져온다
         * @param start 한 주의 시작 요일, 1234560은 각각 월요일부터 일요일까지를 뜻한다
         */
        ...
        nextYear:function(years){
          return _shiftSolarYears(this, _normalizeDelta(years, 'wrong years '));
        },
        nextMonth:function(months){
          return _shiftSolarMonths(this, _normalizeDelta(months, 'wrong months '));
        },
        nextDay:function(days){
          return _shiftSolarDays(this, _normalizeDelta(days, 'wrong days '));
        },
        ...
        getJulianDay:function(){
          return _getJulianDayFromSolarParts(this._p.year, this._p.month, this._p.day, this._p.hour, this._p.minute, this._p.second);
        },
    ...
    var _fromBaZi=function(yearGanZhi,monthGanZhi,dayGanZhi,timeGanZhi,sect,baseYear){
      ...
      var l = [];
      // 월 지지가 인월에서 얼마나 떨어져 있는지에 대한 오프셋
          ...
          if (solarTime.getYear() >= baseYear) {
            ...
            for (var i = 0, j = hours.length; i < j; i++) {
              ...
              if (d === 0 && hour === solarTime.getHour()) {
                // 절령 당일이고 시간까지 정확히 같은 극단적인 경우에는 분과 초도 함께 반영한다
  ...
  var Lunar = (function(){
    ...
    var _computeYear = function(o,solar,year){
      ...
      // 입춘을 새해 시작으로 보는 간지 연호
      var g = yearGanIndex;
      var z = yearZhiIndex;

      // 정확한 간지 연호는 입춘 교체 시각을 기준으로 함
    ...
    var _computeMonth = function(o,solar){
      ...
      // 순번: 대설 이전 -3, 대설~소한 -2, 소한~입춘 -1, 입춘 이후 0
      var index = -3;
      for(i=0;i<size;i+=2){
    ...
    var _fromBridgedLunarResult = function(solar, bridgedLunar) {
      return _new(
        bridgedLunar.year,
        bridgedLunar.intercalation ? -bridgedLunar.month : bridgedLunar.month,
        bridgedLunar.day,
        solar.getHour(),
        solar.getMinute(),
        solar.getSecond(),
        solar,
        LunarYear.fromYear(solar.getYear())
      );
    };
    var _fromBridgedSolarResult = function(lunarYear, lunarMonth, lunarDay, hour, minute, second, bridgedSolar) {
      var solar = Solar.fromYmdHms(bridgedSolar.year, bridgedSolar.month, bridgedSolar.day, hour, minute, second);
      var ly = LunarYear.fromYear(lunarYear);
      if (solar.getYear() !== lunarYear) {
        ly = LunarYear.fromYear(solar.getYear());
      }
      return _new(lunarYear, lunarMonth, lunarDay, hour, minute, second, solar, ly);
    };
    var _fromSolar = function(solar){
      var bridgedLunar = _getKasiHistoricalLunarBySolar(solar.getYear(), solar.getMonth(), solar.getDay());
      if (bridgedLunar) {
        return _fromBridgedLunarResult(solar, bridgedLunar);
      }
      var lunarYear = 0;
      var lunarMonth = 0;
      var lunarDay = 0;
    ...
    var _fromYmdHms = function(lunarYear,lunarMonth,lunarDay,hour,minute,second) {
      var normalized = _normalizeLunarInput(lunarYear, lunarMonth, lunarDay, hour, minute, second);
      lunarYear = normalized.year;
      lunarMonth = normalized.month;
      lunarDay = normalized.day;
      hour = normalized.hour;
      minute = normalized.minute;
      second = normalized.second;
      var bridgedSolar = _getKasiHistoricalSolarByLunar(lunarYear, Math.abs(lunarMonth), lunarDay, lunarMonth < 0);
      if (bridgedSolar) {
        return _fromBridgedSolarResult(lunarYear, lunarMonth, lunarDay, hour, minute, second, bridgedSolar);
      }
  ...
  var SolarWeek = (function(){
    ...
    var _fromYmd = function(y,m,d,start){
      ...
      return {
        ...
        /**
         * 현재 날짜가 해당 월의 몇 번째 주인지 가져온다
         * @return number 주 번호, 1부터 시작
         */
        getIndex:function(){
        ...
        /**
         * 주 단위 이동
         * @param weeks 이동할 주 수, 음수이면 뒤로 이동
         * @param separateMonth 월 단위로 분리해서 계산할지 여부
         * @return object 이동 후의 양력 주
         */
        ...
        /**
         * 이번 주 첫날의 양력 날짜를 가져온다(월이 바뀔 수 있음)
         * @return object 이번 주 첫날의 양력 날짜
         */
        ...
        /**
         * 이번 주 첫날의 양력 날짜를 가져온다(해당 월 내로만 제한)
         * @return object 이번 주 첫날의 양력 날짜
         */
        ...
        /**
         * 이번 주의 양력 날짜 목록을 가져온다(월이 바뀔 수 있음)
         * @return Array 이번 주의 양력 날짜 목록
         */
        getDays:function(){
        ...
        /**
         * 이번 주의 양력 날짜 목록을 가져온다(해당 월 내로만 제한)
         * @return Array 이번 주의 양력 날짜 목록(해당 월 내로만 제한)
         */
  ...
  var SolarSeason = (function(){
    ...
    var _fromYm = function(y,m){
      ...
      return {
        ...
        /**
         * 분기 이동
         * @param seasons 이동할 분기 수, 음수이면 뒤로 이동
         * @return object 이동 후의 분기
         */
  ...
  var SolarHalfYear = (function(){
    ...
    var _fromYm = function(y,m){
      ...
      return {
        ...
        /**
         * 해당 월이 상반기/하반기 중 몇 번째 반기인지 가져온다
         * @return number 반기 번호, 1부터 시작
         */
        getIndex:function(){
        ...
        /**
         * 반기 이동
         * @param halfYears 이동할 반기 수, 음수이면 뒤로 이동
         * @return object 이동 후의 반기
         */
    ...
    var _fromYear = function(lunarYear){
      ...
      return {
        ...
        _compute:function(){
          ...
          // 355는 2000년 12월 동지로, jd에 비교적 가까운 동지 추정값을 얻기 위한 값
          var w = Math.floor((jd - 355 + 183) / 365.2422) * 365.2422 + 355;
          if (ShouXingUtil.calcQi(w) > jd) {
            w -= 365.2422;
          }
          // 25개의 절기 시각(베이징 시간), 동지부터 다음 동지 이후까지
          for (i = 0; i < 26; i++) {
  ...
  var ShouXingUtil = (function(){
    ...
    return {
      ...
      DT_AT:[
        ...
        2012, 66.8, 0.22, 0, 0,
        // 2018, 69.0, 0.36, 0, 0,
        // skyfield의 DE440s △T 예측 데이터를 사용해 보정
  ...
  var SolarUtil = (function(){
    return {
      ...
      isLeapYear:function(year){
        return _isGregorianLeapYear(year);
      },
      getDaysOfMonth:function(year, month){
        return _getSolarDaysOfMonth(year, month);
      },
      getDaysOfYear:function(year){
        return _getSolarDaysOfYear(year);
      },
      getDaysInYear:function(year, month, day){
        return _getSolarDaysInYear(year, month, day);
      },
      getDaysBetween:function(ay, am, ad, by, bm, bd){
        return _getSolarDaysBetween(ay, am, ad, by, bm, bd);
      },
