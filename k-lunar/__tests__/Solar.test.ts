...
test('6', () => {
  ...
  expect(solar.getLunar().toString()).toBe('一〇年腊月初十');
});

test('7', () => {
  var solar = Solar.fromYmd(11,3,1);
  expect(solar.getLunar().toString()).toBe('一一年二月初十');
});

test('9', () => {
  var solar = Solar.fromYmd(26,4,13);
  expect(solar.getLunar().toString()).toBe('二六年三月初十');
});
...
test('20', () => {
  ...
  expect(solar.nextDay(1).toYmd()).toBe('1582-10-05');
});

test('21', () => {
  const solar = Solar.fromYmd(1582, 10, 15);
  expect(solar.nextDay(-1).toYmd()).toBe('1582-10-14');
});

test('22', () => {
  const solar = Solar.fromYmd(1582, 10, 15);
  expect(solar.nextDay(-5).toYmd()).toBe('1582-10-10');
});

test('29', () => {
  const solar = Solar.fromYmd(1397, 5, 15);
  expect(solar.getLunar().toString()).toBe('一三九七年四月初十');
});

test('30', () => {
  const lunar = Solar.fromYmd(1397, 5, 15).getLunar();
  expect(lunar.getSolar().toYmd()).toBe('1397-05-15');
});

test('31', () => {
  const solar = Solar.fromYmd(1022, 3, 11);
  const lunar = solar.getLunar();
  expect(lunar.getYear()).toBe(1022);
  expect(lunar.getMonth()).toBe(1);
  expect(lunar.getDay()).toBe(30);
  expect(lunar.getSolar().toYmd()).toBe('1022-03-11');
});

test('32', () => {
  const solar = Solar.fromDate(new Date('2024-01-01T14:59:59.000Z'));
  expect(solar.toYmdHms()).toBe('2024-01-01 23:59:59');
});

test('33', () => {
  const solar = Solar.fromDate(new Date('2024-01-01T15:00:00.000Z'));
  expect(solar.toYmdHms()).toBe('2024-01-02 00:00:00');
});
